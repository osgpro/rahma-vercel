...
export const getWWWData = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("https://rahmaww.org/api/where-we-work/");
    dispatch({
...
export const getCountryData = (country) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://rahmaww.org/api/where-we-work/${country}`
    );
    dispatch({
...
export const getCountryUpdateData = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://rahmaww.org/api/updates/${id}`);
    dispatch({
